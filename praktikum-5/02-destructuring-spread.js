...
function tambahkanItem(arr, item) { arr.push(item); return arr; }
...
cariMax(Array.from({ length: 1000 }, () => Math.random() * 1000)));

console.log('\n=== 0(n^2) - Bubble Sort (kecil saja) ===');
console.log(bubbleSort([64, 34, 25, 12, 22, 11, 90]));
console.log('\n=== 0(2^n) - Fibonacci Rekursif (n kecil!) ===');
for (let i = 0; i <= 10; i++) process.stdout.write(fibRekursif(i) + ' ');
console.log('');

console.log('\nWaktu fib(35) 0(2^n):');
let t = Date.now(); fibRekursif(35); console.log(Date.now()-t, 'ms');
console.log('\nWaktu fib(35) 0(n) memoization:');
const memo = {};
function fibMemo(n) {
    if (n <= 1) return n;
